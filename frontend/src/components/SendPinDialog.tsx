...
export function SendPinDialog({ pinId, pinTitle, trigger }: { pinId: number; pinTitle: string; trigger: ReactNode }) {
  ...
  const send = useMutation({
    mutationFn: ({ conversationId, name }: { conversationId: number; name: string }) => api.sendMessage(conversationId, `Thought you’d like “${pinTitle}”.`, pinId).then(() => name),
    onSuccess: (name) => {
    ...
    <Dialog.Root open={open} onOpenChange={setOpen}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="dialog-card send-pin-dialog">
          ...
          <Dialog.Description className="muted">Choose a recent conversation. The pin stays attached as a preview in the thread.</Dialog.Description>
