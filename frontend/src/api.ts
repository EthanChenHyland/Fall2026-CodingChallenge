...
export const api = {
  ...
  searchRecommendations: (query = '') => request<{ suggestions: string[]; pins: PublicPin[]; basedOn: string[]; aiEnhanced: boolean }>(`/api/search/recommendations?q=${encodeURIComponent(query)}`),
