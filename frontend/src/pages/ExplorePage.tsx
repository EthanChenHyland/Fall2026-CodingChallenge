...
export function ExplorePage() {
  ...
  return (
    <>
      ...
      <section className="section-head explore-section-head"><div><span className="eyebrow">EXPLORE</span><h2>{mode === 'following' ? 'Fresh saves from people you follow' : 'Fresh saves from public collections'}</h2></div><div className="feed-switch" aria-label="Explore feed"><button className={mode === 'all' ? 'active' : ''} onClick={() => setMode('all')}>For you</button><button className={mode === 'following' ? 'active' : ''} onClick={() => setMode('following')}>Following</button></div></section>
