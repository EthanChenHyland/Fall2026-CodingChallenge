...
type Item = { id: number; collection_id: number; source_id: string; title: string } & Record<string, string | number>
type Snapshot = { item: Item; likes: Array<{ user_id: number; created_at: string }>; comments: Array<{ id: number; user_id: number; body: string; created_at: string }>; cover: boolean }
...
export function snapshotItem(itemId: number, collectionId: number) {
  ...
  db.prepare('INSERT OR REPLACE INTO deleted_items VALUES (?, ?, ?, ?)').run(itemId, collectionId, JSON.stringify({ item, likes, comments, cover }), Date.now() + 10 * 60_000)
}
...
export function restoreSnapshot(collectionId: number, itemId: number): Item | 'duplicate' | null {
  return db.transaction(() => {
    ...
    if (!row) return null
    const { item, likes, comments, cover } = JSON.parse(row.snapshot) as Snapshot
    ...
    db.prepare(`INSERT INTO items (id, collection_id, source_id, image_url, source_page, source_creator, title, note, tags, canvas_x, canvas_y, rotation, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(item.id, collectionId, item.source_id, item.image_url, item.source_page, item.source_creator, item.title, item.note, item.tags, item.canvas_x, item.canvas_y, item.rotation, item.created_at)
    ...
    for (const comment of comments) db.prepare('INSERT INTO comments (id, item_id, user_id, body, created_at) SELECT ?, ?, id, ?, ? FROM users WHERE id = ?').run(comment.id, itemId, comment.body, comment.created_at, comment.user_id)
