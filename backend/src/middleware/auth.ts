...
export type User = { id: number; username: string; name: string; email: string; bio: string; avatar_url: string; created_at: string }
...
export function loadUser(req: AuthedRequest, _res: Response, next: NextFunction) {
  ...
  const row = db.prepare(`
    SELECT u.id, u.username, u.name, u.email, u.bio, u.avatar_url, u.created_at
    FROM sessions s
