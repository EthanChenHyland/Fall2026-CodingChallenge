...
If `OPENROUTER_API_KEY` is configured, typed autocomplete/related-search requests also ask a small language model for closely related visual phrases. That call receives only the typed query and public catalog context; private boards, saved-item metadata, and profile data stay in Mosaic's local recommendation path. The result is cached briefly, bounded by a short timeout, and optional: provider/database suggestions remain the fallback whenever OpenRouter is missing or unavailable. This is a search enhancement, not an autonomous agent.
